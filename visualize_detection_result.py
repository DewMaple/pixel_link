...
def visualize(image_root, det_root, output_root, gt_root=None):
    def read_gt_file(_image_name):
        gt_file = util.io.join_path(gt_root, 'gt_%s.txt' % (_image_name))
        return util.io.read_lines(gt_file)

    def read_det_file(_image_name):
        det_file = util.io.join_path(det_root, 'res_%s.txt' % (_image_name))
        return util.io.read_lines(det_file)

    def read_image_file(_image_name):
        return util.img.imread(util.io.join_path(image_root, _image_name))
